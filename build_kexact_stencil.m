...
for nn = 1:cell.ncells
  ...
  %cells(1:nstencil)=0
  stn_cnt= 1;
  cells = nn; %assign current cell to stencil
  nbrs_list = [nn];
  while (stn_cnt<nstencil)
      ...
      for i = 1:length(cells);
        ...
        nbrs_list = [nbrs_list, cell.nbrs(cur_cell,1:nnbr)];
      end
      cells = unique(nbrs_list);

      stn_cnt = length(cells);
      
%       plot_cells(cell.nodes(cells,2:end),vertex)
...
%     end
  ...
  cell.stencil(nn).cells = cells(1:stn_cnt);

  cell.reconstruction(nn).A=0;
  cell.reconstruction(nn).Ainv=0;
end
...
if any(strcmp(who,'write_sten'))
    ...
    for nn = 1:cell.ncells
      ...
      for i = 1:length(cells)
        ...
        vrtx = cell.nodes(cells(i),2:nvertex+1); %pull off vertex for cell
        ...
        if isempty(vrtx_list) % if there is no current vertex list create one
          ...
          vertex_temp = vertex(vrtx_list,:); % list of node locations
          ...
          cell_temp.nodes(i,1:vrtx_out_cnt) = [vrtx_out_cnt-1, vrtx_out];
      ...
      write_vtk_cv(vertex_temp, cell_temp, fid);
      fclose(fid);
